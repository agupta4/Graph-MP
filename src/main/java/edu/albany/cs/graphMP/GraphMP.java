...
public class GraphMP {
	...
	private double[] run() {
		...
		if (function.getFuncID().equals(FuncType.ProtestScanStat)) {
			x = c;
		}
	...
	/**
	 * in order to fit the gradient, we need to normalize the gradient
	 * if the domain of function is within [0,1]^n
	 * @param x
	 *            input vector x
	 * @param gradient
	 *            gradient vector
