...
public class FastPCST {
    ...
    private ArrayList<PairingHeap.Node> pairing_heap_buffer = new ArrayList<PairingHeap.Node>();    //public heap_buffer (maybe do not need this)
    private ArrayList<EdgePart> edge_parts = new ArrayList<EdgePart>();    //edge parts information
    ...
    private double eps;
    private ArrayList<Boolean> node_good = new ArrayList<Boolean>(); // marks whether a node survives simple pruning
    ...
    public FastPCST(ArrayList<Integer[]> edges, ArrayList<Double> prizes, ArrayList<Double> costs,
                    int root, int target_num_active_clusters, PruningMethod pruningMethod, int verbostiy_level) {
        ...
        for (int ii = 0; ii < edge_info.size(); ++ii) {
            edge_info.get(ii).inactive_merge_event = -1;
        }
        current_time = 0.0;
    ...
    public void get_next_edge_event(Double next_time, Integer next_cluster_index, Integer next_edge_part_index) {
        ...
        next_time = clusters_next_edge_event.get_min_firstP;
        next_cluster_index = clusters_next_edge_event.get_min_secondP;
    ...
    public void get_next_cluster_event(Double next_cluster_time, Integer next_cluster_index) {
        ...
        clusters_deactivation.get_min(next_cluster_time, next_cluster_index);
        next_cluster_time = clusters_deactivation.get_min_firstP;
        next_cluster_index = clusters_deactivation.get_min_secondP;
        this.next_cluster_time = clusters_deactivation.get_min_firstP;
        this.next_cluster_index = clusters_deactivation.get_min_secondP;
    }
    ...
    public void remove_next_cluster_event() {
        ...
        tmp_value = clusters_deactivation.delete_min_firstP;
        tmp_cluster = clusters_deactivation.delete_min_secondP;
    }

    public void get_sum_on_edge_part(int edge_part_index, double total_sum, Double finished_moat_sum, Integer cur_cluster_index, int flag) {
    ...
    public boolean run(ArrayList<Integer> result_nodes, ArrayList<Integer> result_edges) {
        ...
        while (num_active_clusters > target_num_active_clusters) {
            ...
            if (next_edge_time < next_cluster_time) {
                ...
                if (edge_parts.get(next_edge_part_index).deleted) {
                    ...
                    //System.out.println("PCSF Test F:" + stats.total_num_edge_events) ;
                    continue;
                }
                ...
                get_sum_on_edge_part(next_edge_part_index, sum_current_edge_part, current_finished_moat_sum, current_cluster_index, 1);
                if (verbosity_level >= 3) {
                    System.out.format("next_edge_part_index : %d ; sum_current_edge_part : %6f ; current_finished_moat_sum : %6f ; current_cluster_index : %d\n",
                            next_edge_part_index, sum_current_edge_part, current_finished_moat_sum, current_cluster_index);
                }
                sum_other_edge_part = 0.0d;
                other_cluster_index = -1;
                other_finished_moat_sum = -1d;
                get_sum_on_edge_part(other_edge_part_index, sum_other_edge_part, other_finished_moat_sum, other_cluster_index, 2);
                if (verbosity_level >= 3) {
                    System.out.format("other_edge_part_index : %d ; sum_other_edge_part : %6f ; other_finished_moat_sum : %6f ; other_cluster_index : %d\n",
                ...
                if (verbosity_level >= 2) {
                    System.out.format("Edge event at time %6f, current edge part %d (cluster %d), other edge part %d (cluster %d)\n", current_time, next_edge_part_index, current_cluster_index, other_edge_part_index, other_cluster_index);
                ...
                if (remainder < eps * current_edge_cost) {
                    ...
                    other_cluster = clusters.get(other_cluster_index);
                    if (verbosity_level >= 2) {
                        System.out.format("Merge %d and %d into %d\n", current_cluster_index, other_cluster_index, new_cluster_index);
                    }
                    new_cluster.moat = 0.0;
                    new_cluster.prize_sum = current_cluster.prize_sum + other_cluster.prize_sum;
                    new_cluster.subcluster_moat_sum = current_cluster.subcluster_moat_sum + other_cluster.subcluster_moat_sum;
                    ...
                    if (other_cluster.active) {
                        ...
                        if (!other_cluster.contains_root) {
                            ...
                            InactiveMergeEvent merge_event = inactive_merge_events.get(inactive_merge_events.size() - 1);
                            ...
                            edge_info.get(next_edge_part_index / 2).inactive_merge_event = inactive_merge_events.size() - 1;
                        }
                    }
                    other_cluster.merged_into = new_cluster_index;
                    new_cluster.edge_parts = current_cluster.edge_parts.meld(current_cluster.edge_parts, other_cluster.edge_parts);
                    new_cluster.subcluster_moat_sum += current_cluster.moat;
                    new_cluster.subcluster_moat_sum += other_cluster.moat;
                    if (new_cluster.active) {//new_cluster is inactive if new cluster contains root
                        new_cluster.active_start_time = current_time + remainder;
                        double becoming_inactive_time = current_time + remainder + new_cluster.prize_sum - new_cluster.subcluster_moat_sum;
                    ...
                    //do not merge two clusters, but the edge still tight.(since the other edge part is not tight)
                    ...
                    clusters_next_edge_event.delete_element(other_cluster_index);
                    other_cluster.edge_parts.decrease_key(other_edge_part.heap_node, other_cluster.active_start_time + other_edge_part.next_event_val - other_finished_moat_sum, next_event_time);
                    ...
                    tmp_index = current_cluster.edge_parts.get_min_secondP;
                    clusters_next_edge_event.insert(tmp_val, current_cluster_index);
                    other_cluster.edge_parts.decrease_key(other_edge_part.heap_node, other_cluster.active_end_time + other_edge_part.next_event_val - other_finished_moat_sum, other_cluster.active_end_time);
        ...
        if (pruning == PruningMethod.kGWPruning) {
            ...
            for (int ii = phase2_result.size() - 1; ii >= 0; --ii) {
                ...
                if (edge_info.get(cur_edge_index).inactive_merge_event < 0) {
                    ...
                    InactiveMergeEvent cur_merge_event = inactive_merge_events.get(edge_info.get(cur_edge_index).inactive_merge_event);
                    ...
                    if (clusters.get(inactive_cluster_index).necessary) {
                        ...
                        if (verbosity_level >= 2) {
                            System.out.format("One endpoint was inactive but is marked necessary (%d), so keeping edge %d (%d, %d)\n", inactive_cluster_index, cur_edge_index, uu, vv);
                        }
                    } else {
                        mark_nodes_as_deleted(inactive_side_node, active_side_node);
                        if (verbosity_level >= 2) {
                            System.out.format("One endpoint was inactive and not marked necessary (%d), so discarding edge %d (%d, %d)\n", inactive_cluster_index, cur_edge_index, uu, vv);
                        }
                    }
                }
            }
            ...
            strong_pruning_parent = Utils.resize(strong_pruning_parent, prizes.size(), new Pair<Integer, Double>(-1, -1.0d));
            ...
            for (int ii = 0; ii < (int) final_components.size(); ++ii) {
                ...
                if (ii == root_component_index) {
                    ...
                    if (verbosity_level >= 2) {
                        System.out.println("Best start node for current component: " + best_component_root + ", pruning from there\n");
                    }
            ...
            for (int ii = 0; ii < phase2_result.size(); ++ii) {
                ...
                if (node_deleted.get(uu) || node_deleted.get(vv)) {
                    //////////////////////////////////////////
                    if (verbosity_level >= 2) {
                        System.out.println("Not keeping edge " + cur_edge_index + " (" + uu + ", " + vv + ") because at least one endpoint already deleted\n");
            ...
            if (begin) {
                ...
                for (int ii = 0; ii < phase3_neighbors.get(cur_node_index).size(); ++ii) {
                    ...
                    if (next_payoff <= 0.0) {
                        if (mark_as_deleted) {
                            if (verbosity_level >= 2) {
                                System.out.format("Subtree starting at %d has a nonpositive contribution of %6f, pruning (good side: %d)\n", next_node_index, next_payoff, cur_node_index);
                            }
    ...
    public int find_best_component_root(int component_index) {
        ...
        while (!stack2.isEmpty()) {
            ...
            if (parent_val_without_cur_node > parent_edge_cost) {
                ...
                strong_pruning_payoff.set(cur_node_index, currentPayOff + (parent_val_without_cur_node - parent_edge_cost));
            }
    ...
    public void build_phase1_node_set(
            ArrayList<Integer> edge_set, ArrayList<Integer> node_set) {
