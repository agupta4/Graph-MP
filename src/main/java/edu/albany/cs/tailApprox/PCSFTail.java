...
public class PCSFTail {
     ...
     */
    public PCSFTail(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsc,
                    ArrayList<Double> prizesPi, int g, double costBudgetC, double nu, double delta) {
     ...
     * @param B
     */
    public PCSFTail(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsW, double[] z, int s,
                    int g, double B, int[] trueSubGraph) {
        ...
        if (verboseLevel >= 1) {
            ...
            System.out.println("g : " + this.g + " s : " + s + " nu : " + this.nu + " " + " C : " + this.C
                    + " delta : " + this.delta);
        }

        /**
         * try { FileWriter fileWriter = new FileWriter("./tmp/pcsfTail.txt",true) ;
         * fileWriter.write(this.bestForest.nodesInF.size() + " "+ (2*this.nu*s +
         * g)+"\n"); fileWriter.close() ; } catch (IOException e) {
         * e.printStackTrace(); }
         */

        double cT = Math.sqrt(1.0D + 3.0D / (this.nu - 2.0D)); // the parameter of
        // equation 8 is cT >
        // 1 ; cT is
        // arbitrary and
        // fixed constant.
    ...
    private F run() {
        ...
        if (verboseLevel >= 1) {
            System.out.println("pi_min : " + minPi + " lambda0 : " + lambda0 + " c(F) : " + forest.costF
                    + " 2*C : " + 2.0D * C + " pi(\bar{F}) : " + forest.piFBar);
        }
     ...
     * ||b-bSPrime|| is the minimum one. (This means you need to know which subset
     * is optimal)
     *
    ...
    private boolean checkEqu8Valid(double cT, double[] b, double[] bS, double[] bSPrime) {
        ...
        if (b == null || bS == null || bSPrime == null || b.length == 0 || bS.length == 0
                || bSPrime.length == 0) {
            return flag;
        }
        for (int i = 0; i < b.length; i++) {
    ...
    private F PCSF_GW(ArrayList<Integer[]> edges, ArrayList<Double> cLambda, ArrayList<Double> pi,
                      int g) {
        ...
        FastPCST pcstFast = new FastPCST(edges, pi, cLambda, FastPCST.kNoRoot, g,
        ...
        if (!pcstFast.run(resultEdges, nodesInF)) {
            ...
            resultEdges = pcstFast.result_edges; // index
    ...
    public class F {
        ...
        public F(ArrayList<Integer> nodesInF, ArrayList<Integer[]> edgesInF,
                 ArrayList<Double> edgesCostInF, ArrayList<Double> prizePiInF, double totalPrizesInG) {
        ...
        /**
         * using disjoint set data structure to find number of connected component
         * in a forest
         *
        ...
        private int getConnectedComponents() {
            DisjointSet<Integer> dis = new DisjointSet<Integer>(); // need to test
            // further TODO
            for (Integer node : nodesInF) {
                dis.makeSet(node);
            }
        ...
        public ArrayList<Tree> constructTrees() {
            ...
            for (Integer key : componentsMap.keySet()) { // for each component, create
                // a new tree
                Tree tree;
                Set<Integer> nodes = componentsMap.get(key); // nodes in this component
                ArrayList<Integer> nodesInT = new ArrayList<Integer>(nodes); // get all
                // nodes in
                // a tree
                ...
                for (Integer[] edge : this.edgesInF) { // get all edges in a tree
                    if (nodes.contains(edge[0]) || nodes.contains(edge[1])) {
                        ...
                        edgesCostInT.add(this.edgesCostInF.get(index)); // get all edge cost
                        // in a tree
                    }
                }
        ...
        public int[] pruningTrees(double[] y, APDMInputFormat apdm, ArrayList<Integer> tailNodes) {
            ...
            for (Tree tree : this.trees) {
                ...
                while (true) {
                    boolean flag = false;
                    HashMap<Integer, HashMap<Integer, Double>> updatedAdj =
                    ...
                    for (int i : adj.keySet()) {
                        ...
                        if (nei != -1) { // i is a leaf node and i is normal node
                            System.out.println("leaf : " + i + "y[" + i + "]=" + y[i]);
                            if (y[i] == 0.0D) { // i is a leaf node and it is normal node
    ...
    public class Tree implements Comparable<Tree> {
        ...
        private HashMap<Integer, HashMap<Integer, Double>> contructAdj() {
            HashMap<Integer, HashMap<Integer, Double>> adj =
                    new HashMap<Integer, HashMap<Integer, Double>>();
         ...
         * Therefore, we can construct a connected Directed Graph. After that, we
         * can find a tour trough depth first search algorithm.
         *
